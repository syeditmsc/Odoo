...
class PaymentTransaction(models.Model):
    ...
    def send_payment(self):
        ...
        sendpay_data = {
            ...
            "CallBackUrl": f"{odoo_base_url}/payment/worldpay/_return_url",
            "ErrorUrl": f"{odoo_base_url}/payment/worldpay/failed",
            "Language": "en",
        ...
        return {
            'api_url': f"{odoo_base_url}/payment/worldpay/response",
            'data': sendpay_data,
        }

    def _get_tx_from_notification_data(self, provider_code, notification_data):
        """Getting  payment status from worldpay"""
        api_key = self.env['payment.provider'].search(
            [('code', '=', 'worldpay')]).worldpay_token
        base_api_url = self.env['payment.provider'].search(
            [('code', '=', 'worldpay')])._worldpay_get_api_url()
        ...
        tx = super()._get_tx_from_notification_data(provider_code,
                                                    notification_data)
        if provider_code != 'worldpay' or len(tx) == 1:
            return tx
        domain = [('provider_code', '=', 'worldpay')]
        reference = ""

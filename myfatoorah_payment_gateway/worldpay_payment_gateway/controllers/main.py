...
class PaymentworldpayController(http.Controller):
    _return_url = '/payment/worldpay/_return_url'

    @http.route('/payment/worldpay/response', type='http', auth='public',
                website=True, methods=['POST'], csrf=False, save_session=False)
    def worldpay_payment_response(self, **data):
        ...
        return request.render(
            "worldpay_payment_gateway.worldpay_payment_gateway_form", vals)

    @http.route(_return_url, type='http', auth='public',
                methods=['GET'])
    def worldpay_checkout(self, **data):
        """ Function to redirect to the payment checkout"""
        _logger.info("Received worldpay return data:\n%s",
                     pprint.pformat(data))
        tx_sudo = request.env[
            'payment.transaction'].sudo()._get_tx_from_notification_data(
            'worldpay', data)
        tx_sudo._handle_notification_data('worldpay', data)
        return request.redirect('/payment/status')

    @http.route('/payment/worldpay/failed', type='http', auth='user',
                website=True, )
    def payment_failed(self, redirect=None, **data):
        """ Function to render the payment failed cases"""
        return request.render(
            "worldpay_payment_gateway.worldpay_payment_gateway_failed_form")
